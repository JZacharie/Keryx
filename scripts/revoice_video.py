...
async def generate_cloned_speech(text, output_path):
    ...
    # Standard GPT-SoVITS API follows this or similar:
    # URL is usually / or /tts
    try:
...
def translate_segments(segments, translator):
    print("--- Phase 1: Translating segments (French to English) ---")
    for seg in segments:
        text = seg["text"].strip()
        if text:
            translated = translator(text)[0]['translation_text']
            seg["translated_text"] = translated
            print(f"FR: {text} -> EN: {translated}")
        else:
            seg["translated_text"] = ""
    return segments
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 revoice_video.py <manifest_json> <keyframes_folder> [output_mp4]")
        sys.exit(1)

    m_path = sys.argv[1]
    k_folder = sys.argv[2]
    out_path = sys.argv[3] if len(sys.argv) > 3 else "/app/outputs/dubbed_video_cloned.mp4"
